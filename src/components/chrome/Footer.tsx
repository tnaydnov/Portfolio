...
export function Footer() {
    ...
    <div className={styles.footerDirectory}>
      <div className={styles.footerIdentity}><Link href={href("/")} className={styles.footerName}>{site.name}<span aria-hidden="true">↗</span></Link><p>{site.role}</p><a href={`mailto:${site.email}`} dir="ltr">{site.email}</a></div>
