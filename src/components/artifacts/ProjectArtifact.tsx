...
const STUDIES = {
  arc: {
    ...
    note: { en: "Prepare. Teach. Learn. Repeat.", he: "להכין. ללמד. ללמוד. לשפר." },
    label: { en: "Arc: three connected learning stages form a continuous feedback loop", he: "Arc: שלושה שלבי למידה מחוברים במעגל של משוב ושיפור" },
  },
  applytide: {
    title: "Applytide", index: "02", theme: "apply", color: "#ffad91",
    note: { en: "A clearer path through the search.", he: "דרך ברורה יותר בתוך החיפוש." },
    label: { en: "Applytide: scattered application records align into an organized workflow", he: "Applytide: רשומות מועמדות מפוזרות מסתדרות בתהליך עבודה מאורגן" },
  },
  eventa: {
    title: "Eventa", index: "03", theme: "event", color: "#c9b5f5",
    note: { en: "A shared place. A new connection.", he: "מקום משותף. חיבור חדש." },
    label: { en: "Eventa: two guest profiles connect within an illustrative event network", he: "Eventa: שני פרופילי אורחים מתחברים בתוך רשת אירוע להמחשה" },
  },
  "license-plate-recognition": {
    title: "License Plate Recognition", index: "04", theme: "recognition", color: "#a7d8ef",
    note: { en: "From a frame to a readable result.", he: "מתמונה לתוצאה קריאה." },
    label: { en: "License plate recognition: detection, recognition and output shown as three processing planes", he: "זיהוי לוחיות רישוי: איתור, זיהוי ופלט בשלושה מישורי עיבוד" },
  },
  "trading-system": {
    title: "Trading System", index: "05", theme: "trading", color: "#e8ce9a",
    note: { en: "Permissions. Actions. State.", he: "הרשאות. פעולות. מצב." },
    label: { en: "Trading system: connected structures represent permissions, purchases and state", he: "מערכת מסחר: מבנים מחוברים מייצגים הרשאות, רכישות ומצבים" },
  },
} as const;

type ArtifactSlug = keyof typeof STUDIES;
type ProjectArtifactProps = { slug: string; locale: Locale; size?: "card" | "hero" };
type SceneProps = { id: string; locale: Locale };

function Platform({ x, y, width = 230, depth = 110, height = 30, fill = "#203a33" }: { x: number; y: number; width?: number; depth?: number; height?: number; fill?: string }) {
  const side = depth * .52;
...
function ArcStudy({ id, locale }: SceneProps) {
  const words = locale === "he" ? ["הכנה", "כיתה", "משוב"] : ["Preparation", "Classroom", "Feedback"];
  return <g>
...
function ApplyStudy({ id, locale }: SceneProps) {
  const words = locale === "he" ? ["שמירה", "הגשה", "מעקב"] : ["Capture", "Apply", "Follow through"];
  return <g>
...
function EventStudy({ id, locale }: SceneProps) {
  ...
  return <g>
    ...
    <g className={styles.svgLabel} textAnchor="middle"><text x="601" y="602">{locale === "he" ? "אורחים לדוגמה / אירוע משותף" : "Illustrative guests / Shared event"}</text></g>
  </g>;
}

function RecognitionStudy({ id, locale }: SceneProps) {
  return <g>
    ...
    <g className={styles.svgLabel} textAnchor="middle"><text x="350" y="511">01 / {locale === "he" ? "איתור" : "Detect"}</text><text x="609" y="544">02 / {locale === "he" ? "זיהוי" : "Recognize"}</text><text x="868" y="577">03 / {locale === "he" ? "פלט" : "Output"}</text></g>
  </g>;
}

function TradingStudy({ id, locale }: SceneProps) {
  return <g>
    ...
    <g className={styles.svgLabel} textAnchor="middle"><text x="416" y="521">{locale === "he" ? "הרשאות" : "Permissions"}</text><text x="646" y="564">{locale === "he" ? "רכישה" : "Purchase"}</text><text x="854" y="518">{locale === "he" ? "מצב" : "State"}</text></g>
  </g>;
}
...
export function ProjectArtifact({ slug, locale, size = "card" }: ProjectArtifactProps) {
  ...
  const caption = slug === "license-plate-recognition"
    ...
    : slug === "eventa" ? locale === "he" ? "תרחיש להמחשה" : "Illustrative scenario"
      : locale === "he" ? "המחשת מוצר" : "Product illustration";
  return <div className={`${styles.artifact} ${styles[study.theme]}`} data-size={size} role="img" aria-label={`${study.label[locale]}. ${caption}.`}>
    <div className={styles.stage} aria-hidden="true">
      ...
      <svg viewBox="0 0 1200 700" className={styles.scene} fill="none" aria-hidden="true" focusable="false">
        ...
        <Scene id={id} locale={locale} />
      </svg>
      <div className={styles.studyNote}><span>{study.note[locale]}</span><i aria-hidden>↗</i></div>
    </div>
  </div>;
}
